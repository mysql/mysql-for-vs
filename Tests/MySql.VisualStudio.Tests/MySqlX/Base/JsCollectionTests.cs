...
{
  public abstract class JsCollectionTests : BaseTests
  {
    ...
    /// </summary>
    protected const string ADD_JSON_DOCUMENT1 = "var result = coll.add({ name: 'my first', passed: 'document', count: 1}).execute();";

    /// <summary>
    /// Statement to insert a record to the test table
    /// </summary>
    protected const string ADD_JSON_DOCUMENT2 = "var result = coll.add({ name: 'my second', passed: 'again', count: 2}).execute();";

    /// <summary>
    /// Statement to insert multiple records at the same time on a single statement to the test collection
    /// </summary>
    protected const string ADD_MULTIPLE_DOCUMENTS_MULTIPLE_ADD_STATEMENTS = "var result = coll.add({name: 'my sixth', passed: 'aaaand again', count: 6}).add({name: 'my seventh', passed: 'aaaand once again', count: 7}).execute();";

    /// <summary>
    /// Statement to insert multiple records at the same time on a single statement to the test collection
    /// </summary>
    protected const string ADD_MULTIPLE_DOCUMENTS_SINGLE_ADD_STATEMENT = "var result = coll.add([{name: 'my third', passed: 'once again', count: 3},{name: 'my fourth', passed: 'and again', count: 4}, {name: 'my fifth', passed: 'and finally', count: 5}]).execute();";
    ...
    //protected const string _deleteCollectionTest = "session." + _testCollectionName + ".drop();";
    /// <summary>
    ...
    /// </summary>
    protected const string FIND_SPECIFIC_DOCUMENT_TEST = "var records = coll.find('name like \"my second\"'); records.execute();";

    /// <summary>
    /// Statement to update a record in the test table in a single command
    /// </summary>
    protected const string MODIFY_DOCUMENT = "var modr = coll.modify(\"name like 'my fourth'\"); var setr = modr.set('name','dummy'); var sortr = setr.sort(['name']); sortr.execute();";

    /// <summary>
    /// Statement to delete a record in the test table in a single command
    /// </summary>
    protected const string REMOVE_DOCUMENT = "var crud = coll.remove(\"name like 'my fifth'\"); crud.execute();";

    /// <summary>
    /// Statement to select the update record from the test table
    /// </summary>
    protected const string SELECT_UPDATED_RECORD = "var urecord = coll.find('name like \"dummy\"'); urecord.execute();";

    /// <summary>
    /// Get and set the test table
    /// </summary>
    protected const string SET_COLLECTION_VAR = "var coll = session." + TEST_SCHEMA_NAME + ".getCollection('" + TEST_COLLECTION_NAME + "');";

    /// <summary>
    /// Get and set the test database
    /// </summary>
    protected const string SET_SCHEMA_VAR = "var schema = session.getSchema('" + TEST_SCHEMA_NAME + "');";
