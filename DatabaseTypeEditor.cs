...
namespace MySql.Data.MySqlClient.Design
{
	...
	/// </summary>
	internal class DatabaseTypeEditor : UITypeEditor
	{
		...
		public override object EditValue(ITypeDescriptorContext pContext, IServiceProvider pProvider, object pValue)
		{
			if (pContext != null
				...
				try
				{
					MySqlConnectionString settings = (MySqlConnectionString)pContext.Instance;
		...
		protected void List_Click(object pSender, EventArgs pArgs)
		{
			...
			databaseName = (string)list.Items[ list.SelectedIndex ];
		}  

		protected void FillInList(MySqlConnectionString settings)
		{
			list.Items.Clear();

			try 
			{
				MySqlConnection c = new MySqlConnection( settings.CreateConnectionString() );
				c.Open();
				MySqlCommand cmd = new MySqlCommand("SHOW DATABASES", c);
			...
			{
				MessageBox.Show("Unable to show databases");
			}
		}
