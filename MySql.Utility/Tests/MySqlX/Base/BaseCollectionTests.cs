  ...
  public abstract class BaseCollectionTests : BaseTests
  {
    ...
    public void Modify()
    {
      ...
      try
      {
        ...
        if (docResult != null)
        {
          ...
          foundRatingList = ((object[]) foundValue)?.ToList();
        }
        ...
        {
          docResult.TryGetValue("status", out foundValue);
          docResult.TryGetValue("ratings", out var foundValue2);
          foundRatingList = ((object[])foundValue2)?.ToList();
        }
    ...
    public void TestSessions()
    {
      ...
      try
      {
        ...
        sessionIsOpen = sessionIsOpen.Contains("true") ? "true" : "false";
        Assert.True(sessionIsOpen != null && bool.TryParse(sessionIsOpen, out _));

        // Parse Uri of active session
        var shellParseSessionResult = ExecuteQuery(CollectionTestProps.ShellParseUriFromSessionUri);
        Assert.True(shellParseSessionResult != null && shellParseSessionResult.Contains("dbUser"));
      }
      finally
      {
        Command?.Dispose();
