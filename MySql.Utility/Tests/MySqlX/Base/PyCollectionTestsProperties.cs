...
{
  /// <summary>
  /// Defines properties specific to collections tests in Python.
  /// </summary>
  public class PyCollectionTestsProperties : CollectionTestsProperties
  {
    ...
    /// </summary>
    public override string CreateCollection { get; } = "session.get_schema('{0}').create_collection('{1}')";

    /// <summary>
    /// Gets a statement to create a non-unique index.
    /// </summary>
    public override string CreateNonUniqueIndexMovies { get; } = "coll.create_index('" + BaseCollectionTests.MOVIES_NON_UNIQUE_INDEX_NAME + "').field('rating', 'text(5)', True).execute()";

    /// <summary>
    /// Gets a statement to create a schema.
    /// </summary>
    public override string CreateSchema { get; } = "session.create_schema('{0}')";

    /// <summary>
    /// Gets a statement to create a unique index.
    /// </summary>
    public override string CreateUniqueIndexMovies { get; } = "coll.create_index('" + BaseCollectionTests.MOVIES_UNIQUE_INDEX_NAME + "', mysqlx.IndexType.UNIQUE).field('title', 'text(255)', True).execute()";

    /// <summary>
    /// Gets a statement to drop a collection.
    /// </summary>
    public override string DropCollection { get; } = "session.drop_collection('{0}', '{1}')";

    /// <summary>
    /// Statement to drop non-unique index.
    /// </summary>
    public override string DropNonUniqueIndexMovies { get; } = "coll.drop_index('" + BaseCollectionTests.MOVIES_NON_UNIQUE_INDEX_NAME + "').execute()";

    /// <summary>
    /// Gets a statement to drop the test database
    /// </summary>
    public override string DropSchema { get; } = "session.drop_schema('{0}')";

    /// <summary>
    /// Statement to drop unique index.
    /// </summary>
    public override string DropUniqueIndexMovies { get; } = "coll.drop_index('" + BaseCollectionTests.MOVIES_UNIQUE_INDEX_NAME + "').execute()";

    /// <summary>
    /// Gets a specific colletion and assign it to a variable for persistence. 
    /// </summary>
    public override string GetCollection { get; } = "coll = session.get_schema('{0}').get_collection('{1}')";
    ...
    /// </summary>
    public override string ModifyArrayAppendUser { get; } = "coll.modify('name like :param1').array_append('ratings', 'PG-13').bind('param1', 'Iggy%').execute()";

    /// <summary>
    /// Statement to modify a SakilaX.User value and insert a value to a specific array in a specified position.
    /// </summary>
    public override string ModifyArrayDeleteUser { get; } = "coll.modify('name like :param1').array_delete('ratings[2]').bind('param1', 'Iggy%').execute()";

    /// <summary>
    /// Statement to modify a SakilaX.User value and insert a value to a specific array in a specified position.
    /// </summary>
    public override string ModifyArrayInsertUser { get; } = "coll.modify('name like :param1').array_insert('ratings[2]', 'G').bind('param1', 'Iggy%').execute()";
    ...
    /// </summary>
    public override string IsSessionOpen { get; } = "session.is_open()";

    /// <summary>
    /// Gets a statement to get the parsed Uri of the active session Uri.
    /// </summary>
    public override string ShellParseUriFromSessionUri { get; } = "shell.parse_uri(session.get_uri())";

    #endregion Session Queries

    /// <summary>
    /// Initializes a new instance of the <see cref="PyCollectionTestsProperties"/> class.
    /// </summary>
    /// <param name="scriptLanguage">A <see cref="ScriptLanguageType"/> value.</param>
    public PyCollectionTestsProperties(ScriptLanguageType scriptLanguage)
      : base(scriptLanguage)
    {
