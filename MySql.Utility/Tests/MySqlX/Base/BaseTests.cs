...
{
  public abstract class BaseTests : IUseFixture<SetUpBaseShell>, IDisposable
  {
    ...
    /// <summary>
    /// Gets or sets the <see cref="MySqlBaseShell"/> to execute commands using the X Protocol.
    /// </summary>
    private MySqlBaseShell _baseShell;
    ...
    /// </summary>
    public SetUpBaseShell SetUp { get; protected set; }
    ...
    public virtual void SetFixture(SetUpBaseShell data)
    {
      SetUp = data;
      ...
      XConnString = $"{SetUp.UserName}:{SetUp.Password}@{SetUp.HostName}:{SetUp.XPort}";
    }
    ...
    protected string ExecuteQuery(string statement)
    {
      switch(Xecutor)
      {
        case XecutorType.XProxy:
        ...
        case XecutorType.BaseShell:
          switch (ScriptLanguage)
          {
            case ScriptLanguageType.JavaScript:
              return _baseShell.ExecuteToJavaScript(statement);

            case ScriptLanguageType.Python:
              _baseShell.Execute(statement);
              return _baseShell.ResultMessage;
          }
    ...
    protected List<Dictionary<string, object>> ExecuteSingleStatement(string statement)
    {
      ...
      {
        case XecutorType.XProxy:
          return _xProxy.ExecuteSingleStatementAsResultObject(statement, ScriptLanguage);
        ...
        case XecutorType.BaseShell:
          ...
          {
            case ScriptLanguageType.JavaScript:
              result = _baseShell.ExecuteToJavaScript(statement);
              break;

            case ScriptLanguageType.Python:
              _baseShell.Execute(statement);
              result = _baseShell.ResultMessage;
              break;
          }

          var resultObject = ExtensionMethods.ToBaseShellResultObject(result);
          return resultObject.ToDictionariesList(out _); 
      }
    ...
    /// <summary>
    /// Frees resources on the <see cref="_baseShell"/>.
    /// </summary>
    private void DisposeBaseShell()
    {
      _baseShell?.Dispose();
    }
    ...
    /// <summary>
    /// Initializes the <see cref="MySqlBaseShell"/> instance with common statements.
    /// </summary>
    private void InitBaseShell()
    {
      DisposeBaseShell();
      _baseShell = new MySqlBaseShell();
      _baseShell.MakeConnection(XConnString);
      _baseShell.SwitchMode(ScriptLanguage.ToMode());
      _baseShell.AppendAdditionalModulePaths(ScriptLanguage);
    }
