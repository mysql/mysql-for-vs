...
namespace MySql.Utility.Tests.MySqlX.Base
{
  public abstract class BaseTableTests : BaseTests
  {
    ...
    /// </summary>
    protected const int CHARACTERS_MALE_COUNT = 19; //TODO: Bug #2: Revert back to 20

    /// <summary>
    /// The number of non-base characters with an age > 30.
    /// </summary>
    protected const int CHARACTERS_NON_BASE_AGE_GREATER_THAN_30_COUNT = 4; //TODO: Bug #2 : Revert back to 5
    ...
    /// <summary>
    /// Statement to insert a couple of records specifying the columns as an array.
    /// </summary>
    ...
    /// <summary>
    /// Statement to insert a couple of records specifying the columns as a comma delimited list.
    /// </summary>
    ...
    /// <summary>
    /// Statement to insert a couple of records without specifying the columns, just values.
    /// </summary>
    ...
    public virtual void Delete()
    {
      ...
      try
      {
        ...
        ExecuteQuery(INSERT_COLUMNS_AS_ARRAY3); //TODO: Bug #2: Generating duplicate error.
        ...
        charactersCount += 7; //TODO: Bug #2: Revert back to 8
        selectResult = ExecuteSingleStatement(SELECT_ALL_TABLE);
        Assert.True(selectResult != null, string.Format(NULL_OBJECT, "selectResult"));
    ...
    public virtual void Insert()
    {
      ...
      try
      {
        ...
        ExecuteQuery(INSERT_COLUMNS_AS_ARRAY3); //TODO: Bug #2: Generating duplicate error.
        ExecuteQuery(INSERT_COLUMNS_AS_ARRAY4);
        charactersCount += 1; //TODO: Bug #2: Revert back to 2
        selectResult = ExecuteSingleStatement(SELECT_ALL_TABLE);
        Assert.True(selectResult != null, string.Format(NULL_OBJECT, "selectResult"));
        Assert.True(selectResult != null && selectResult.Count == charactersCount, DATA_NOT_MATCH);
    ...
    public void Views()
    {
      ...
      try
      {
        ...
        var result = ExecuteQuery(TableTestProps.TableIsView).Contains("true") ? "true" : "false";
        ...
        result = ExecuteQuery(TableTestProps.TableIsView).Contains("true") ? "true" : "false";
        Assert.True(result != null && Convert.ToBoolean(result));
